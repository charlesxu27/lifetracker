...
[I found myself comfortable with React and the front-end this week. However, I am still getting the hang of user authentication and starting the process of creating a user token.]

* If you had more time, what would you have done differently? Would you have added additional features? Changed the way your project responded to a particular event, etc.
  
[If I had more time, I would try to add more error handling and responsiveness to the login and registration forms because I know the majority of user interaction starts there.]

* Reflect on your project demo, what went well? Were there things that maybe didn't go as planned? Did you notice something that your peer did that you would like to try next time?

[I was able to showcase all my features, and I am happy with my demo.]
...
[Shout out to my podmates Anessa and Christy for their patience and help along the way!]
...
- To build out the front-end, start with the `App.jsx` component:
  ...
  - [x] Create an **`auth`** context:
    ...
    - [x] In `App.jsx` file create an `AppContainer` component that wraps the `App` component with the `AuthContextProvider` component (which should still be nested inside the `BrowserRouter` component from `react-router-dom`). Export the `AppContainer` component by default instead of the `App`x
...
- **Authentication**
  - [x] Go ahead andxbuild out a full-fledged authentication flow using PostgreSQL, `bcrypt`, and JSON Web Tokens. For it all to work, we'll need a `User` model, a `security` middleware, some `tokens` utility functions, and the appropriate `auth` routes.
...
- **Resources and Permissions**
  ...
  - [x] The **/nutrition** routes
    ...
    - [ ] In the `routes/nutrition.test.js` file:
      ...
      - [ ] Test xhe `POST /nutrition` endpoint
        - [ ] Write test cases for:
